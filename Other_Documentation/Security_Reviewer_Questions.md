...
# What attacks remain?
Here're some attacks that will remain. 
...
- Sandboxing related attacks: Linux's ```seccomp, namespaces, AppArmor, SELinux``` or Windows' ```AppContainer, Windows Sandbox technologies, Restricted tokens / Job Objects (depending on the application)``` security sandboxing measures may be absent or not yet enable on them.
